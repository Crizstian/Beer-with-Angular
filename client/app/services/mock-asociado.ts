import {Cervecero} from '../models/cervecero';

export var ASOCIADOS:Cervecero[] = [
  {
    id: 1,
    nombre: 'Cinco Mulatas',
    logo: 'asociados/La_mulata/1.jpg',
    texto: 'CincoMulatas Es una marca de cerveza  completamente familiar, nacida en el centro histórico de morelia e inspirada en la vieja leyenda de "La Pila de La  Mulata".',
    imagenes : [
      "asociados/La_mulata/2.jpg"
    ],
    contacto: {
     telefono: '4432139906',
     correo: 'Lamulatamorelia@hotmail.com'
   }
 },
 {
   id: 2,
   nombre: 'Rodilla del Diablo',
   logo: "asociados/Rodilla_del_Diablo/logo.jpg",
   texto: 'Somos una cervecería local e independiente, producimos cervezas artesanales empleando solamente ingredientes naturales de la cerveza.',
   imagenes : [
     "asociados/Rodilla_del_Diablo/botella.jpg"
   ],
   contacto: {
    telefono: '4521169797',
    correo: 'rdcerveceria@hotmail.com'
  }
},
{
  id: 3,
  nombre: 'Cerveza Xakúa',
 logo: "asociados/Xakua/logo.jpg",
 texto: 'Cerveza artesanal moreliana, realizamos 3 estilos: Stout de Avena, Stout de Café de olla y Ambar Ale.',
 imagenes : [
   "asociados/Xakua/botella.jpg"
 ],
 contacto: {
   telefono: '4432269017',
   correo: 'cerveza.xakua@hotmail.com'
 }
},
{id: 4,
 nombre: 'Nacional Morelos',
logo: "asociados/Nacional_Morelos/logo.jpg",
texto: 'Cerveceria Nacional Morelos es una marca dedicada a cervezas artesanales faciles al paladar, elaboradas en Morelia, Michoacán.',
imagenes : [
  "asociados/Nacional_Morelos/botella.jpg"
],
contacto: {
  telefono: '4433127839',
  correo: 'contacto@nacionalmorelos.com.mx'
}
},
{id: 5,
nombre: 'Malora',
logo: "asociados/Malora/logo.jpg",
texto: `Malora ha surgido como una pequeña casa cervecera con grandes cervezas, reconocida en el mercado local por producirlas, sobre todo,
para conocedores, que aprecian los sabores intensos y tienen gusto por el arte y la gastronomía.
En casa, nos vemos a nosotros mismos como los primeros clientes, con un sentido autocrítico, pues amamos lo que hacemos y
queremos compartirlo con todos.`,
imagenes : [
 "asociados/Malora/botella.jpg"
],
contacto: {
 telefono: '4432732825',
 correo: 'ahmed@malora.com.mx'
}
},
{id: 6,
nombre: 'Cueva Santa Cervecería',
logo: "asociados/Cueva_Santa/logo.jpg",
texto: `Somos una empresa 100% michoacana , la cual se dedica a realizar cerveza artesanal con la finalidad de poder llevar a sus paladares

sabores a los cuales están poco familiarizados pero con ingredientes 100% naturales, nuestra razón de ser es siempre estar llevando

cerveza de la mas alta calidad ante el mercado.`,
imagenes : [
 "asociados/Cueva_Santa/botella.jpg"
],
contacto: {
 telefono: '4432812813',
 correo: 'cuevasantam@gmail.com'
}
},
{id: 7,
nombre: 'Rastrera',
logo: "asociados/Rastrera/logo.jpg",
texto: `En cervecería Rastrera nos preocupamos por elaborar cervezas de calidad utilizando, en la medida de lo posible, insumos nacionales.

De igual manera, buscamos promover el consumo de calidad y responsable de la cerveza, de manera que se aprecie los  matices de aroma,

sabor y color que presentan los diferentes estilos de cervezas.`,
imagenes : [
 "asociados/Rastrera/botella.jpg"
],
contacto: {
 telefono: '4431941527',
 correo: 'rastrera.cerveza@yahoo.com'
}
},
{id: 8,
nombre: 'Libertadores',
logo: "asociados/Libertadores/logo.png",
texto: `Cerveza Libertadores da la posibilidad a nuestros consumidores de liberar su paladar con auténtica cerveza.

Nuestra marca mixta está representada por el águila real mexicana, la cuál es una de las aves que vuelan más alto.

Al ser considerada la reuna de la saves, tienen la posiblidad de volar libremente.

Aquí es donde se da una analogía con quienes prueban nuestra cerveza. Nuestros consumidor busca nuevos horizontes y

deciden ellos mismos liberar su paladar con nuevas experiencias, en este caso con nuestra marca.`,
imagenes : [
 "asociados/Libertadores/botella.jpg"
],
contacto: {
 telefono: '013535333067.',
 correo: 'ventas@cervezalibertadores.com'
}
},
{
  id: 9,
  nombre: 'Demonio',
  logo: "asociados/Demonio/logo.jpg",
  texto: `Cerveza originaria de Uruapan, ofrecemos 4 estilos: Demonio Blanco (Hefeweizen), Demonio Rojo (Irish Red Ale), Demonio Café (Brown Porter) y Demonio Verde (American IPA).`,
  imagenes : [
    "asociados/Demonio/botella.jpg"
  ],
  contacto: {
    telefono: '4521715506',
    correo: 'ventas@casacerveceralucas.mx'
  }
},
{
  id: 10,
  nombre: 'Zurumato',
  logo: "asociados/Zurumuato/1.jpg",
  texto: 'Antiguo nombres de pastor Ortiz en el bajío michoacano, lugar que significa lugar de paja o entre paja.',
  imagenes : [
    "asociados/Zurumuato/2.jpg"
  ],
  contacto: {
    telefono: '7224409271',
    correo: 'c.zurumuato@hotmail.com'
  }
},
{
  id: 11,
  nombre: 'Tepoli',
  logo: "asociados/Tepoli/logo.jpg",
  texto: `Empresa con principios a la cultura nacional pre y post-colombina, resaltando sabores y tradiciones mexicanas.`,
  imagenes : [
    "asociados/Tepoli/botella.jpg"
  ],
  contacto: {
    telefono: '4521057057',
    correo: 'saulrodriguez@tepolicerveceria.com'
  }
}];
